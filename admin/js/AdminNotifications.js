...
function send_suppliers() {
    ...
    ref.orderByChild('accountType').equalTo('Supplier').on('value', function (snapshot) {
        ...
        if (a) {
            ...
            snapshot.forEach(function (childSnapshot) {
                ...
                var key = firebase.database().ref().child('Pickly').child('notificationRequestsfortest3').child(childSnapshot.child('id').val()).push();

                suppliers[suppliersindex] = childSnapshot.child('id').val();
                suppliersindex++;

                var map2 = {
                    action: Action, datee: format, from: id, isRead: "false", orderid: "", statue: text, to: childSnapshot.child('id').val(), uName: "Quicker", ppURL: "https://firebasestorage.googleapis.com/v0/b/pickly-ed2f4.appspot.com/o/ppUsers%2Ficon.png?alt=media&token=4dce9f34-2794-4690-b888-10e5b0cb7865"
                };


                ref = firebase.database().ref().child('Pickly').child('notificationRequestsfortest3').child(childSnapshot.child('id').val()).child(key.key).set(map2); 
    ...
    ref.orderByChild('accountType').equalTo('Delivery Worker').on('value', function (snapshot) {
        ...
        if (a) {
            ...
            snapshot.forEach(function (childSnapshot) {
                ...
                var map3 = {
                    action: Action2, datee: format, from: id, isRead: "false", orderid: "", statue: text2, to: childSnapshot.child('id').val(), uName: "Quicker", ppURL: "https://firebasestorage.googleapis.com/v0/b/pickly-ed2f4.appspot.com/o/ppUsers%2Ficon.png?alt=media&token=4dce9f34-2794-4690-b888-10e5b0cb7865"
                };
